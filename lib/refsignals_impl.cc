...
// void dvbt_pilot_gen::set_channel_gain(int spilot, gr_complex val, float snr)
void dvbt_pilot_gen::set_channel_gain(int spilot, gr_complex val, float snr)
{
    ...
    float received_power = std::norm(gr_complex((4 * 2 * (0.5 - d_wk[spilot]) / 3), 0));
    ...
    // // Estimate the noise power from the received signal power
    // double noise_power = rx_power.mag2() - tx_power.mag2();

    // // Calculate the MMSE estimate of the channel gain
    // d_channel_gain[spilot] = tx_power / (rx_power + error * error.conj() + noise_power);
...
int dvbt_pilot_gen::process_spilot_data(const gr_complex* in, float snr)
// int dvbt_pilot_gen::process_spilot_data(const gr_complex* in)
{
    ...
    for (int i = 0, startk = d_chanestim_carriers[0]; i < d_chanestim_index; i++) {
        ...
        set_channel_gain(k, in[k + d_zeros_on_left], snr);
                                ...
                                float snr)
{
    ...
    int diff_symbol_index = process_spilot_data(d_derot_in.data(), snr);
...
refsignals::sptr refsignals::make(int itemsize,
                             int ninput,
                             int noutput,
                             float snr,
                             ...
                             int cell_id)
{
    return gnuradio::make_block_sptr<refsignals_impl>(itemsize,
                                                    ninput,
                                                    noutput,
                                                    snr,
                                                    constellation,
 ...
 */
refsignals_impl::refsignals_impl(int itemsize,
    int ninput,
    int noutput,
    float snr,
    dvb_constellation_t constellation,
 ...
 */
refsignals_impl::~refsignals_impl() {}

void refsignals_impl::forecast(int noutput_items, gr_vector_int& ninput_items_required)
{
    // #pragma message( 
...
int refsignals_impl::general_work(int noutput_items,
